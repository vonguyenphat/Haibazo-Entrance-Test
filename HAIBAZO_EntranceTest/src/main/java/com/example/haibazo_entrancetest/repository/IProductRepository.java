...
public interface IProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "SELECT new com.example.haibazo_entrancetest.dto.ProductFindAllDTO (" +
            ...
            "ORDER BY sku2.price ASC LIMIT 1 )) " +
            "FROM Product p " +
            "JOIN SKUs sku ON sku.product.id = p.id " +
            ...
            "GROUP BY p.id " +
            "HAVING ((:option_1 IS NOT NULL AND :option_2 IS NOT NULL AND :option_3 IS NOT NULL AND COUNT(DISTINCT pvo.name) = 3) " +
            "       OR (:option_1 IS NOT NULL AND :option_2 IS NOT NULL AND :option_3 IS NULL AND COUNT(DISTINCT pvo.name) = 2) " +
            "       OR (:option_1 IS NOT NULL AND :option_2 IS NULL AND :option_3 IS NULL AND COUNT(DISTINCT pvo.name) = 1) " +
